...
MODEL_GEMMA = 'gemma3:4b'  # Default Gemma model
MODEL_LLAVA = 'llava'
MODEL_OFF = 'off'          # Off state value
WEBCAM_INDEX = 0
FRAME_WIDTH = 640
FRAME_HEIGHT = 480
...
server_data = {
    ...
    "action_result": "",
    "live_faces": [],
    "model": MODEL_GEMMA  # Default model state
}

# --- Global ML State (Loaded once) ---
known_face_encodings = []
known_face_names = []
analysis_results = {} # Shared dict for analysis threads
action_thread = None       # Holds the live action comprehension thread
stop_action_thread = False # Flag to tell the thread to stop
...
def action_comprehension_thread():
    """
    Runs in the background *while* recording is active.
    Uses the *currently selected* model from server_data.
    """
    ...
    while not stop_action_thread:
        ...
        if is_keyframe:
            ...
            with data_lock:
                server_data["keyframe_count"] = keyframe_count
            ...
            try:
                ...
                with data_lock:
                    server_data["action_result"] = "Error connecting to Ollama."
                time.sleep(2)
...
def analyze_frame_with_gemma(frame, name):
    """
    Analyzes a single frame, writes result to analysis_results dict.
...
# --- Video Processing Thread (Updated) ---
def video_processing_thread():
    """
    (Updated) Main background thread for processing webcam video.
    """
    ...
    while True:
        ...
        # --- Keyframe Logic (Now checks is_recording) ---
        if is_recording:
            gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
            gray = cv2.GaussianBlur(gray, (21, 21), 0)
            # ... (rest of keyframe logic is unchanged) ...
            if last_action_frame_gray is None:
                last_action_frame_gray = gray
                b64_frame = frame_to_base64(frame)
                if b64_frame:
                    action_frames.append(b64_frame) # This list is now local
            ...
            with data_lock:
                server_data["keyframe_count"] = len(action_frames)
        ...
        if frame_count % PROCESS_EVERY_NTH_FRAME == 0:
            ...
            if face_locations:
                ...
                for face_location, face_encoding in zip(face_locations, face_encodings):
                    ...
                    # --- ✨ --- UPDATED: Trigger single-frame analysis --- ✨ ---
        ...
        # --- Data Sync & Drawing (runs every frame) ---
        live_face_payload = []
        ...
        for (face_location, name, confidence) in current_face_data:
            # ✨ --- NEW: Clear analysis if model is off or recording --- ✨
...
@app.route('/get_data')
def get_data():
    """(Unchanged) Provides the live data to the frontend."""
...
def toggle_action():
    """(Updated) Handles the button click to start/stop the *live action thread*."""
    ...
    with data_lock:
        ...
        current_model = server_data['model'] # Get current model
        ...
        if server_data["is_recording"]:
            ...
            if current_model == MODEL_OFF:
                ...
                server_data["is_recording"] = False # Toggle it back off
            else:
                print("[Action Analysis] STARTING live comprehension thread.")
                stop_action_thread = False
                action_frames = [] # Clear frames
                last_action_frame_gray = None # Clear last frame
            ...
            if action_thread is not None:
                stop_action_thread = True # Signal the thread to stop
                action_thread = None
...
def set_model():
    """(Updated) Handles the model switch from the frontend."""
    global server_data, data_lock
    
    data = request.json
    model_name = data.get('model', MODEL_GEMMA) 
    
    if model_name not in [MODEL_GEMMA, MODEL_LLAVA, MODEL_OFF]:
        model_name = MODEL_GEMMA
        
    with data_lock:
        server_data['model'] = model_name
        # ✨ --- NEW: If turning off, also stop any live recording --- ✨
...
if __name__ == "__main__":
    # Load the faces once at startup
    load_known_faces(KNOWN_FACES_DIR)
    
    # Start the background webcam processing thread
